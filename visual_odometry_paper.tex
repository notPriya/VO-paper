...
We propose one framework that can estimate the motion of multiple types of robots. The method is general and robust enough to work on different platforms in different situations.
...
Another approach for localization of tethered robots is to measure the payout length of the tether. Using the tether for positioning is feasible if the structure of the environment is linear and known beforehand.

For localization in more varied environments, dead reckoning uses encoder information to estimate the robot's velocity and, via integration, the robot's position. For many robots, dead reckoning produces poor estimates because it does not account for slip.
